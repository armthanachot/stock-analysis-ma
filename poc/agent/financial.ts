...
import { financialTools, getStockInsight, getStockNews, getStockPrice } from "../../src/pkg/ai/tools/financial";
...
import { technicalTools, getStockTechnical } from "../../src/pkg/ai/tools/technical";
...
// Query-based schema selection
const getResponseSchema = (query: string) => {
    const q = query.toLowerCase();
    
    // Technical Analysis queries
    if (q.includes('แนวรับ') || q.includes('แนวต้าน') || q.includes('support') || 
        q.includes('resistance') || q.includes('technical') || q.includes('rsi') || 
        q.includes('macd') || q.includes('ema') || q.includes('tf ')) {
        return {
            type: "object",
            additionalProperties: false,
            properties: {
                analysis: baseAnalysisSchema,
                priceAnalysis: priceAnalysisSchema,
                technicalAnalysis: technicalAnalysisSchema,
                indicatorAnalysis: indicatorAnalysisSchema,
                metadata: metadataSchema
            },
            required: ["analysis", "priceAnalysis", "technicalAnalysis", "indicatorAnalysis", "metadata"]
        };
    }
    
    // Fundamental Analysis queries
    if (q.includes('งบการเงิน') || q.includes('fundamental') || q.includes('pe') || 
        q.includes('eps') || q.includes('revenue') || q.includes('profit')) {
        return {
            type: "object",
            additionalProperties: false,
            properties: {
                ...
                analystInsights: analystInsightsSchema,
                metadata: metadataSchema
            },
            required: ["analysis", "priceAnalysis", "fundamentalAnalysis", "analystInsights", "metadata"]
        };
    }
    
    // News Analysis queries
    if (q.includes('ข่าว') || q.includes('news') || q.includes('sentiment')) {
        return {
            type: "object",
            additionalProperties: false,
            properties: {
            ...
            required: ["analysis", "priceAnalysis", "newsAnalysis", "metadata"]
        };
    }
    
    // Default: Full schema for comprehensive analysis
    return fullSchema;
};

// Keep full schema for reference
const fullSchema = {
    type: "object",
    additionalProperties: false,
    properties: {
...
const analyze = async (i: string) => {
    const instructions = `
        ...
        - please response in valid JSON format
        - don't contain any other text or explanation
        - don't contain markdown format
...
console.log(JSON.stringify(await analyze(`PLTR น่าสนใจมั้ย?`), null, 2))